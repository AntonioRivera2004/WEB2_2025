...
function eliminarTarea()
{
    ...
    {   
        let mensaje="\n";
        tareas.forEach((tarea,index)=>{mensaje+=`${index+1}.-${tarea.nombre}\n`})
        let numero = parseInt(prompt("Ingresa numero de tarea a eliminar: "+mensaje));
        if(numero > 0 && numero<=tareas.length)
        {
        tareas.splice(numero-1,1); //Se elimina la tarea seleccionada del arreglo
        alert("Tarea Eliminada para siempre :D")
        }else{
...
//Crear una función para que se llame eliminarTarea (usar la función marcar tareacompletada para punto de partida, pero al buscar la tarea la eliminamos,
//  cambiamos el push por un eliminar (Quitarlo del arreglo))